...
abstract class TestComponent {
  ...
  data = Array(ITEMS_COUNT)
    .fill(0)
    .map((_, i) => ({ id: i }));
  data2 = Array(ITEMS_COUNT2)
    .fill(0)
    .map((_, i) => ({ id: i + ITEMS_COUNT }));
...
function runTableTests(tableModule: typeof CdkTableModule, tableComponent: Type<TestComponent>) {
  describe('common actions', () => {
    ...
    it('should set the correct rendered range on scroll', () => {
      ...
      cy.get('.wrapper')
        ...
        .wait(0)
        .then(() => {
          expect(viewport.getRenderedRange()).to.deep.equal({
            start: rowsToScroll - bufferSize,
            end: rowsToScroll + VISIBLE_ITEMS_COUNT + bufferSize,
          });
        });
    });

    it('should subscribe and rerender after dataSource is changed', () => {
      cy.get('tbody').children().first().should('contain', 'el - 0');

      cy.get('[data-cy="changeDataSource"]').click();

      cy.get('tbody').children().first().should('contain', `el - ${ITEMS_COUNT}`);
    });

    it('should check scroll position after dataSource is changed', () => {
      cy.get('tbody').children().first().should('contain', 'el - 0');

      cy.get('.wrapper').scrollTo('bottom').wait(0);
    ...
    tests.forEach(test => {
      it('should have visible ' + test.title, () => {
        ...
        if (test.props.headerEnabled) {
          cy.get('th').should((test.checks.header ? '' : 'not.') + `be.inViewport`, '.wrapper');
        }
        if (test.props.footerEnabled) {
          cy.get('.footer-cell').should((test.checks.footer ? '' : 'not.') + `be.inViewport`, '.wrapper');
        }
      });
    });
  });
